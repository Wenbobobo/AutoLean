...
_SPECS: tuple[dict[str, Any], ...] = (
    ...
    {
        "sample_id": "mg-a-finite-noetherian-compactness",
        "domain": "mg-a",
        "source": "Every finite metric space is compact, and every Noetherian topological space is quasi-compact.",
        "normalized": "For every finite metric space X, conclude X is compact; and for every Noetherian topological space Y, conclude Y is quasi-compact.",
        "quantifiers": (
            "for every finite metric space X",
            "for every Noetherian topological space Y",
        ),
        "assumptions": (
            "X is a metric space with finitely many points",
            "Y is a Noetherian topological space",
        ),
        "conclusion": "X is compact and Y is quasi-compact",
        "illustrative_snippet_a": "theorem mg_a_finite_noetherian_compactness (X Y : Type) [MetricSpace X] [TopologicalSpace Y] : Finite X -> IsNoetherianTopology Y -> IsCompact (Set.univ : Set X) /\\ IsQuasiCompact (Set.univ : Set Y) := by exact finite_and_noetherian_compactness",
        "illustrative_snippet_b": "example (X Y : Type) [MetricSpace X] [TopologicalSpace Y] : Finite X -> IsNoetherianTopology Y -> CompactSpace X /\\ QuasiCompactSpace Y := by exact finite_noetherian_compactness",
        "reverse": "Finiteness is the stated route to compactness for X, while the Noetherian condition is the stated route to quasi-compactness for Y; neither clause asserts a converse.",
        "positive": "A finite metric space has a finite subcover for every open cover, and a Noetherian space has no infinite strictly ascending chain of open subsets, which yields quasi-compactness.",
        "negative": "The real line is a metric space that is not compact, and an infinite discrete topological space is not quasi-compact.",
        "ambiguity": "Compactness is used for the metric-space clause and quasi-compactness for the topological-space clause; this synthetic sample does not identify either notion with a Lean definition.",
        "mutations": (
            {
                "code": "DROP_FINITE_METRIC_COMPACTNESS",
                "difference_kind": "drop_finite",
                "probe_kind": "drop_finite",
                "baseline": "For every finite metric space X",
                "mutated": "For every metric space X",
                "reason": "Removing finiteness upgrades the compactness claim to all metric spaces; the real line is a counterexample.",
            },
            {
                "code": "DROP_NOETHERIAN_QUASICOMPACTNESS",
                "difference_kind": "drop_noetherian",
                "probe_kind": "drop_noetherian",
                "baseline": "for every Noetherian topological space Y",
                "mutated": "for every topological space Y",
                "reason": "Removing the Noetherian condition upgrades the quasi-compactness claim to all topological spaces; an infinite discrete space is a counterexample.",
            },
        ),
    },
    {
