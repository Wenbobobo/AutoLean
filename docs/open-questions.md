...
| OQ-008 | Has the HF incident been contained, and which credential rotations are complete? | Operator confirms the HF archive was deleted; treat any historically exposed credential as exposed; do not migrate raw data | Archive deletion does not prove provider-side access state or rotation | Dataset and credential owners | Deletion confirmed; provider/rotation containment open |
...
| OQ-010 | Which first Builder pilot source/domain has clear rights and expert availability? | Conditional model-theory sequent-calculus soundness only; run self-calibration and a pinned-Library spike before ingestion | Phase 2 needs a selected, calibratable boundary rather than merely available text | Builder lead plus experts | Candidate split recorded; rights/expert gates open |
...
| OQ-017 | Does the closed-only or structural/open-formula first-order soundness candidate compile in the pinned `Library` environment without scope loss? | Retain both candidates, resolve the quantifier/freshness gap, and do not start manual Builder calibration | This determines whether the primary is faithful enough to test and whether a backup must be selected | Library steward, Builder lead, domain reviewer | Both semantic adapters compiled in a local WSL spike; a tracked canonical receipt and the self-calibration record record `partial_passed_with_gap`. The round deliberately remains incomplete/gap and all semantic-review gates remain open. No candidate is selected |
...
OQ-016. OQ-017 additionally blocks the current pilot's manual-calibration and ingestion path.
