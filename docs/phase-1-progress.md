...
Snapshot: 2026-07-25 baseline, with a 2026-07-27 to 2026-07-28 working-tree addendum and a
2026-07-29 remote-candidate addendum below
...
## 2026-07-29 active working-tree status addendum
...
| DeepSeek ten-call role observation | Ten frozen, no-retry role calls settled under the fixed 256-token output ceiling. Every accepted completion reached that ceiling; the redacted record is non-promotable. | Saturation prevents a competence conclusion, role-floor admission, benchmark score, proof claim, or Builder-fidelity inference. |
| DeepSeek output-budget ablation | A versioned 256-vs-512 protocol passed zero-call preflight. Its fresh run dispatched one 256-token control request, recorded redacted `network`, made no retry, skipped the candidate arm, and emitted no comparison. The public report SHA-256 is `d49f0fe94cfbd7cdc162c5bc9a4a49c6c3b2f3e907dbba6c82b9d6c206654a31`. | This is transport-interruption evidence only. It establishes no budget effect, competence, role-floor admission, cost, proof, or Builder-fidelity result. |
...
| External-model access | The operator supplied an experimental DeepSeek credential through an ignored local secret file. Ten fixed 256-token role calls later settled, but all saturated; a separate 512-token ablation stopped after one redacted network failure and emitted no comparison. | The settled saturated run and partial transport run are both non-promotable. Neither is independent capability evidence, semantic review, proof evidence, or production evaluator authority. |
...
| Controlled model/benchmark evaluation | Authorized bootstrap, ten-trial role, structural grammar, and output-budget ablation paths are implemented. Ten 256-token calls settled but saturated; the 512-token ablation stopped after one network failure and produced no comparison. | Independent capability evidence, production admission/evaluator custody, an unsaturated retained comparison, and frozen evaluator authority are still required. No model or FATE result closes a Builder or Library gate. |
