...
| P2-13 | Partial | The [versioned machine-advisory admission sidecar](builder-machine-advisory-admission.md) now requires complete per-reviewer risk assessments and treats any semantic escalation, critical dissent, surviving mutation, counterexample, a shared declared quorum failure domain, missing or shared model/sidecar-failure-domain lineage, and missing, mismatched, future, or stale calibration as `abstain`. The source-free [next-case intent queue](research/ifem-next-calibration-case-intents-v1.md) retains 21 stable `not_authored` work intents from the D35 route (`P1=10`, `P2=2`, `P3=9`) without source, Lean, oracle, provider, or private payload. A [fake-first authoring harness](research/ifem-source-free-case-authoring-v1.md) now routes only those nine P3 intents through 3/3/3 finite partitions and 27 formalizer/reviewer/supervisor calls; its retained [plan](research/ifem-source-free-case-authoring-plan-2026-07-31.json) and [report](research/ifem-source-free-case-authoring-fake-report-2026-07-31.json) explicitly declare public replayability, no held-out isolation, same-agent abstention, all authority false, and no freeze or Prover handoff. This is authored project-synthetic plumbing, not iFEM statement content, private benchmarking, model work, or calibration evidence. Current quorum execution, declared lineage, and digest-only calibration remain unverified. |
...
| **M4 — Builder calibration factory** | Rights-cleared textbook openings; 50--100 non-frozen candidates; independent extraction/formalization/critique/falsification and calibration measurement. | The eleven-sample synthetic opening corpus has a source-span multi-role fake harness and a deterministic 5/3/3 held-out structural protocol. The iFEM risk route now also has a nine-case, 27-stage source-free fake authoring harness with strict finite role projections and exact run binding. All are architecture regressions; the new 3/3/3 labels are publicly replayable topology, not private held-out or semantic calibration. | Measured harmful-negative resistance, abstention behavior, real private-seed/recovery isolation, and source-specific rights boundary. Machine output remains `machine_advisory`. |
