...
Remote facts checked on 2026-08-01: public draft
[PR #26](https://github.com/Wenbobobo/AutoLean/pull/26) is open at
`69fdf34af39be1d06054c9fa7a8936718055682f`, and all four jobs in GitHub Actions
[run 30673227320](https://github.com/Wenbobobo/AutoLean/actions/runs/30673227320) succeeded. A
separately retained project-local clean-stage record for that exact commit reports 2,228 passed, 13
explicit environment/cache/OCI skips, and 0 failures; the current PR description is not the source
of that count. CI success and a project-authored local record do not independently establish
semantic fidelity, a Lean result, provider authority, or production authority. Earlier `cd42ba7`
and `f251e19` candidates remain historical receipts, not current heads.
...
| P2-04 | Partial | Thirteen pinned iFEM source files are locally locked and independently replayed under `local_only`, with source-lock receipt SHA-256 `74eca6689fe69dcbf2f34ea524a99cacc2054c0a39cfecfb11887c29e13cf239`. Digest-only indexes bind 161 notebook cells and three `intro.md` heading sections. The fixed opening Markdown has a separately replayed byte-identical text overlay, and four opening notebook Markdown cells have write-once private logical-text projections that replay against the unchanged source-lock candidate and index. The public [coarse calibration plan](research/ifem-coarse-local-calibration-plan-v1.md) binds exactly those four containers and the pending local-use request while remaining text-free and non-executable. Its follow-on local-use object is an unattested `unknown` candidate with no permitted endpoint. The [atomic-span protocol](research/ifem-atomic-source-span-v1.md) validates exact UTF-8 consensus, private persistence, and redaction only on project-synthetic text; it exposes no real iFEM or provider entry point. A deterministic [candidate-only dependency graph](ifem-candidate-dependency-graph.md) records 25 nodes and 49 planning edges without source text. This closes source/locator, local text preparation, plan selection, graph-skeleton construction, and synthetic atomic-span mechanics only: the four cells are not mathematical claim spans, and there is no model-egress authorization, semantic source mapping, mathematical dependency claim, FormalGraph, ExecutionGraph, freeze, or Prover handoff. |
...
| **M1 — Reproducible Phase 1 architecture** | Weeks 1--12: control plane, fake/provider adapters, FATE boundary, dashboard, chaos receipt, release/public scans. | Substantially complete in local scope; PR #26 at `69fdf34af39be1d06054c9fa7a8936718055682f` is green in CI run `30673227320`. | A no-RC/RC decision names every gate. Local/CI evidence alone is not a semantic, kernel, signer, or model result. |
...
| **M4 — Builder calibration factory** | Rights-cleared textbook openings; 50--100 non-frozen candidates; independent extraction/formalization/critique/falsification and calibration measurement. | The synthetic opening corpus and iFEM source-free lane include private seed/ledger plus a ModelWork/authorization/CAS-bound sidecar with conservative recovery. The first-case successor exercises formalizer, reviewer, and cheating-supervisor predecessor mechanics under a three-attempt fake-transport ceiling, but no three-role real-provider observation exists. The real iFEM opening has four replayed private cell projections and a public text-free coarse plan. Its rights object remains an unattested `unknown` candidate. Project-synthetic M4.8 tests exercise atomic UTF-8 boundaries, exact dual-output consensus, typed abstention, persist-before-project recovery, and public redaction, but still produce no real atomic claim span or executable source-backed run. | Authorized repeated live execution, measured harmful-negative resistance and abstention behavior, independently attested isolation, and source-specific rights boundary. Machine output remains `machine_advisory`. |
