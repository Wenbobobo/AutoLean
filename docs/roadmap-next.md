...
| P2-13 | Partial | The [versioned machine-advisory admission sidecar](builder-machine-advisory-admission.md) now requires complete per-reviewer risk assessments and treats any semantic escalation, critical dissent, surviving mutation, counterexample, a shared declared quorum failure domain, missing or shared model/sidecar-failure-domain lineage, and missing, mismatched, future, or stale calibration as `abstain`. The source-free [next-case intent queue](research/ifem-next-calibration-case-intents-v1.md) now retains 21 stable `not_authored` work intents from the D35 route (`P1=10`, `P2=2`, `P3=9`) without source, Lean, oracle, provider, or private payload. It is scheduling metadata, not authored cases or calibration evidence. Current quorum execution, declared lineage, and digest-only calibration remain unverified; authority stays `machine_advisory`, freeze is false, and Prover handoff is forbidden. |
