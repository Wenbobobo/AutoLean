...
`3a2cbfffe2d99689ef4082d5f45bd7a04ebf4e27`, and all four jobs in GitHub Actions
[run 30669222958](https://github.com/Wenbobobo/AutoLean/actions/runs/30669222958) succeeded. A
separately retained project-local clean-stage record for that exact commit reports 2,154 passed, 13
...
| **M1 — Reproducible Phase 1 architecture** | Weeks 1--12: control plane, fake/provider adapters, FATE boundary, dashboard, chaos receipt, release/public scans. | Substantially complete in local scope; PR #26 at `3a2cbfffe2d99689ef4082d5f45bd7a04ebf4e27` is green in CI run `30669222958`. | A no-RC/RC decision names every gate. Local/CI evidence alone is not a semantic, kernel, signer, or model result. |
...
| **M4 — Builder calibration factory** | Rights-cleared textbook openings; 50--100 non-frozen candidates; independent extraction/formalization/critique/falsification and calibration measurement. | The synthetic opening corpus and iFEM source-free lane include private seed/ledger plus a ModelWork/authorization/CAS-bound counting-fake sidecar with conservative recovery. The real iFEM opening has four replayed private cell projections and a public text-free coarse plan. Its rights object remains an unattested `unknown` candidate. Project-synthetic M4.8 tests now exercise atomic UTF-8 boundaries, exact dual-output consensus, typed abstention, persist-before-project recovery, and public redaction, but still produce no real atomic claim span or executable source-backed run. | Authorized repeated live execution, measured harmful-negative resistance and abstention behavior, independently attested isolation, and source-specific rights boundary. Machine output remains `machine_advisory`. |
