...
| P2-13 | Partial | The [versioned machine-advisory admission sidecar](builder-machine-advisory-admission.md) now requires complete per-reviewer risk assessments and treats any semantic escalation, critical dissent, surviving mutation, counterexample, a shared declared quorum failure domain, missing or shared model/sidecar-failure-domain lineage, and missing, mismatched, future, or stale calibration as `abstain`. The source-free [next-case intent queue](research/ifem-next-calibration-case-intents-v1.md) retains 21 stable `not_authored` work intents from the D35 route (`P1=10`, `P2=2`, `P3=9`) without source, Lean, oracle, provider, or private payload. A [fake-first authoring harness](research/ifem-source-free-case-authoring-v1.md) routes only those nine P3 intents through 3/3/3 finite partitions and 27 formalizer/reviewer/supervisor calls; its retained [plan](research/ifem-source-free-case-authoring-plan-2026-07-31.json) and [report](research/ifem-source-free-case-authoring-fake-report-2026-07-31.json) explicitly declare public replayability, no held-out isolation, same-agent abstention, all authority false, and no freeze or Prover handoff. The [operator-private seed V2](research/ifem-source-free-private-seed-v2.md) persists a repository-external private manifest before public projection while leaving entropy/storage/isolation claims false or unverified. The [private stage ledger V1](research/ifem-source-free-stage-ledger-v1.md) now consumes only that persisted seed plus exact queue replay and enforces 27 ordered write-once counting-executor coordinates with conservative recovery. These remain project-synthetic plumbing, not iFEM statement content, private benchmarking, provider execution, or calibration evidence. Current quorum execution, declared lineage, ModelWork/authorization/CAS sidecar, and digest-only calibration remain unverified. |
...
| **M1 — Reproducible Phase 1 architecture** | Weeks 1--12: control plane, fake/provider adapters, FATE boundary, dashboard, chaos receipt, release/public scans. | Substantially complete in local scope; PR #26 at `007895dc2fa4f0dd379529e85e3715dd30bccc0e` is green in CI run `30630305418`. | A no-RC/RC decision names every gate. Local/CI evidence alone is not a semantic, kernel, signer, or model result. |
...
| **M4 — Builder calibration factory** | Rights-cleared textbook openings; 50--100 non-frozen candidates; independent extraction/formalization/critique/falsification and calibration measurement. | The eleven-sample synthetic opening corpus has a source-span multi-role fake harness and a deterministic 5/3/3 held-out structural protocol. The iFEM risk route now also has a nine-case fake authoring harness, operator-private seed V2 store, and counting-fake 27-stage private ledger. The latter two fix private bytes, role dependencies, and callback/recovery accounting but do not attest entropy, provider transport, storage, worker isolation, or model eligibility. All remain architecture regressions, not private held-out or semantic calibration. | ModelWork/authorization/CAS-bound execution, measured harmful-negative resistance and abstention behavior, independently attested isolation, and source-specific rights boundary. Machine output remains `machine_advisory`. |
